...
@RestController
@RequestMapping("sas")
public class SasDownloadController {
    @Value("${sasFilePath}")
    String filePath;
    @Value("${sasFileName}")
    String projectName;
    private final Logger logger = LoggerFactory.getLogger(SasDownloadController.class);
    /**
     * 获取所有的日志文件名称
     */
    ...
    @ResponseBody
    public String downloadLogs(HttpServletRequest request,HttpServletResponse response) throws ParseException {
        return FileUtils.downloadLogs(response,filePath,"sas");
    }
    /**
     * 检测系统是否正常
     */
    ...
    public String downloadByDate(HttpServletRequest request,HttpServletResponse response) throws ParseException{
        FileUtils.downloadByDate(request, response, filePath, projectName);
        return "loading.......";
    }
}
